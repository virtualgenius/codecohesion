...
function createRoutes() {
    ...
    const dataLoader = new data_loader_1.DataLoader();
    const queryService = new query_service_1.QueryService(dataLoader);
    /**
     * GET /api/repos
    ...
    router.get('/repos', async (req, res) => {
        try {
            const { url } = req.query;
            if (url) {
                const repo = await queryService.findRepoByUrl(url);
                if (!repo) {
                    const allRepos = await dataLoader.listRepos();
                    const error = (0, error_helper_1.createRepoNotFoundError)(url, allRepos);
    ...
    router.get('/repos/:repoId/stats', async (req, res) => {
        try {
            ...
            const allRepos = await dataLoader.listRepos();
            const errorResponse = (0, error_helper_1.createRepoNotFoundError)(req.params.repoId, allRepos);
        ...
        try {
            ...
            if (limit) {
                ...
                if (isNaN(limitNum) || limitNum < 1 || limitNum > 100) {
                    const error = (0, error_helper_1.createInvalidParameterError)('limit', limit, 'must be between 1 and 100', `https://codecohesion-api.railway.app/api/repos/${repoId}/contributors?limit=10`);
                    return res.status(400).json(error);
                }
            }
            const contributors = await queryService.getContributors(repoId, since, until, limitNum);
    ...
    router.get('/contributors', async (req, res) => {
        try {
            ...
            if (!url) {
                const error = (0, error_helper_1.createMissingParameterError)('url', 'string', 'GitHub repository URL (e.g., https://github.com/facebook/react)', 'https://codecohesion-api.railway.app/api/contributors?url=https://github.com/facebook/react&days=30&limit=5');
                return res.status(400).json(error);
            }
            const repo = await queryService.findRepoByUrl(url);
            if (!repo) {
            ...
            if (limit) {
                limitNum = parseInt(limit, 10);
                if (isNaN(limitNum) || limitNum < 1 || limitNum > 100) {
                    const error = (0, error_helper_1.createInvalidParameterError)('limit', limit, 'must be between 1 and 100', `https://codecohesion-api.railway.app/api/contributors?url=${encodeURIComponent(url)}&days=90&limit=10`);
                    return res.status(400).json(error);
                }
            }
            ...
            const contributors = await queryService.getContributors(repo.id, sinceDate, until, limitNum);
            res.json({
                ...contributors,
    ...
    router.get('/repos/:repoId/files', async (req, res) => {
        try {
            ...
            const files = await queryService.getFiles(repoId, path, metric);
            res.json(files);
        }
        catch (error) {
     ...
     */
    router.get('/repos/:repoId/hotspots', async (req, res) => {
        try {
            const { repoId } = req.params;
            const { limit } = req.query;
            const limitNum = limit ? parseInt(limit, 10) : 20;
            if (limitNum < 1 || limitNum > 100) {
                const error = (0, error_helper_1.createInvalidParameterError)('limit', limitNum, 'must be between 1 and 100', `https://codecohesion-api.railway.app/api/repos/${repoId}/hotspots?limit=20`);
                return res.status(400).json(error);
            }
